...
- If the apex domain is serving stale content after a new deployment, purge the Pages cache or redeploy. The `_headers` file disables caching for `index.html`, so the page should refetch on every visit once the new build is available.
